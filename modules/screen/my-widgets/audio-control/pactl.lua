...
function pactl.volume_increase(device, step)
  ...
  if pactl.get_volume(device) > config.volume_max then
    spawn('pactl set-sink-volume ' .. device .. ' ' .. tostring(config.volume_max) .. '%', false)        
  end
end
...
function pactl.get_volume(device)
  ...
  for vol in string.gmatch(stdout, "(%d?%d?%d)%%") do
    local vol_num = tonumber(vol)
    if vol_num ~= nil then
      volsum = volsum + vol_num
      volcnt = volcnt + 1
    end
  end
